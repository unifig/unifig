...
/**
 * 活跃用户周留存 统计; st_active_user_retain_ws_yyyymmdd
 *
 */
@Service
public class St_Active_User_Retain_Ws extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(St_Active_User_Retain_Ws.class);
	private static final String configJsonPath = "classpath*:conf/json/st_active_user_retain_ws.json";
	private static final String select_source_storeId = "${store_id}";

	private DataSourcePool dataSourcePool;

	public String getName() {
		return "st.active_user_retain_ws";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_ws_sql)) {
				target_ws_sql = target_ws_sql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + ScriptTimeUtil.optime_monday());
			...
			if (!StringUtil.isNullOrEmpty(insertWsSql)) {
				insertWsSql = insertWsSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + ScriptTimeUtil.optime_monday());
			}
			// 获取周留存查询SQL
			...
			// 查询活跃用户时间
			String statisDateSql = "select statis_date from bi_tmp.tmp_st_active_user_retain_ws_all_store group by statis_date";
			...
			for (int retainTime : retainTimes) {
				...
				if (!StringUtil.isNullOrEmpty(drop_ws_tmp_yyyymmdd)) {
					drop_ws_tmp_yyyymmdd = drop_ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_week);
				...
				if (!StringUtil.isNullOrEmpty(create_ws_tmp_yyyymmdd)) {
					create_ws_tmp_yyyymmdd = create_ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_week);
				...
				if (!StringUtil.isNullOrEmpty(insert_ws_old_yyyymmdd)) {
					insert_ws_old_yyyymmdd = insert_ws_old_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_week);
					insert_ws_old_yyyymmdd = insert_ws_old_yyyymmdd.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_week);
				...
				for (String retainDate : retainUserDate) {
					...
					case "2":
						...
						if (!StringUtil.isNullOrEmpty(insert_2ws_new_yyyymmdd)) {
							insert_2ws_new_yyyymmdd = insert_2ws_new_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_2week);
						}
						// 获取周留存查询SQL
					...
					case "3":
						...
						if (!StringUtil.isNullOrEmpty(insert_3ws_new_yyyymmdd)) {
							insert_3ws_new_yyyymmdd = insert_3ws_new_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_3week);
						}
						// 获取周留存查询SQL
					...
					case "4":
						...
						if (!StringUtil.isNullOrEmpty(insert_4ws_new_yyyymmdd)) {
							insert_4ws_new_yyyymmdd = insert_4ws_new_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_4week);
						}
						// 获取周留存查询SQL
					...
					case "5":
						...
						if (!StringUtil.isNullOrEmpty(insert_5ws_new_yyyymmdd)) {
							insert_5ws_new_yyyymmdd = insert_5ws_new_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_5week);
						}
						// 获取周留存查询SQL
					...
					case "6":
						...
						if (!StringUtil.isNullOrEmpty(insert_6ws_new_yyyymmdd)) {
							insert_6ws_new_yyyymmdd = insert_6ws_new_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_6week);
						}
						// 获取周留存查询SQL
					...
					case "7":
						...
						if (!StringUtil.isNullOrEmpty(insert_7ws_new_yyyymmdd)) {
							insert_7ws_new_yyyymmdd = insert_7ws_new_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_7week);
						}
						// 获取周留存查询SQL
					...
					case "8":
						...
						if (!StringUtil.isNullOrEmpty(insert_8ws_new_yyyymmdd)) {
							insert_8ws_new_yyyymmdd = insert_8ws_new_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_8week);
						}
						// 获取周留存查询SQL
					...
					case "9":
						...
						if (!StringUtil.isNullOrEmpty(insert_9ws_new_yyyymmdd)) {
							insert_9ws_new_yyyymmdd = insert_9ws_new_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_9week);
						}
						// 获取周留存查询SQL
			...
			if (!retainUserDate.isEmpty()) {
				for (String retainDate : retainUserDate) {
					...
					case "2":
						String retains_hour_2week = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_active_user_retain_ws_" + retains_hour_2week, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert2WsSelectSql)) {
							insert2WsSelectSql = insert2WsSelectSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_2week);
							insert2WsSelectSql = insert2WsSelectSql.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_2week);
						...
						if (!StringUtil.isNullOrEmpty(drop_2ws_tmp_yyyymmdd)) {
							drop_2ws_tmp_yyyymmdd = drop_2ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_2week);
					...
					case "3":
						String retains_hour_3week = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_active_user_retain_ws_" + retains_hour_3week, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert3WsSelectSql)) {
							insert3WsSelectSql = insert3WsSelectSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_3week);
							insert3WsSelectSql = insert3WsSelectSql.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_3week);
						...
						if (!StringUtil.isNullOrEmpty(drop_3ws_tmp_yyyymmdd)) {
							drop_3ws_tmp_yyyymmdd = drop_3ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_3week);
					...
					case "4":
						String retains_hour_4week = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_active_user_retain_ws_" + retains_hour_4week, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert4WsSelectSql)) {
							insert4WsSelectSql = insert4WsSelectSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_4week);
							insert4WsSelectSql = insert4WsSelectSql.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_4week);
						...
						if (!StringUtil.isNullOrEmpty(drop_4ws_tmp_yyyymmdd)) {
							drop_4ws_tmp_yyyymmdd = drop_4ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_4week);
					...
					case "5":
						String retains_hour_5week = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_active_user_retain_ws_" + retains_hour_5week, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert5WsSelectSql)) {
							insert5WsSelectSql = insert5WsSelectSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_5week);
							insert5WsSelectSql = insert5WsSelectSql.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_5week);
						...
						if (!StringUtil.isNullOrEmpty(drop_5ws_tmp_yyyymmdd)) {
							drop_5ws_tmp_yyyymmdd = drop_5ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_5week);
					...
					case "6":
						String retains_hour_6week = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_active_user_retain_ws_" + retains_hour_6week, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert6WsSelectSql)) {
							insert6WsSelectSql = insert6WsSelectSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_6week);
							insert6WsSelectSql = insert6WsSelectSql.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_6week);
						...
						if (!StringUtil.isNullOrEmpty(drop_6ws_tmp_yyyymmdd)) {
							drop_6ws_tmp_yyyymmdd = drop_6ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_6week);
					...
					case "7":
						String retains_hour_7week = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_active_user_retain_ws_" + retains_hour_7week, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert7WsSelectSql)) {
							insert7WsSelectSql = insert7WsSelectSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_7week);
							insert7WsSelectSql = insert7WsSelectSql.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_7week);
						...
						if (!StringUtil.isNullOrEmpty(drop_7ws_tmp_yyyymmdd)) {
							drop_7ws_tmp_yyyymmdd = drop_7ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_7week);
					...
					case "8":
						String retains_hour_8week = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_active_user_retain_ws_" + retains_hour_8week, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert8WsSelectSql)) {
							insert8WsSelectSql = insert8WsSelectSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_8week);
							insert8WsSelectSql = insert8WsSelectSql.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_8week);
						...
						if (!StringUtil.isNullOrEmpty(drop_8ws_tmp_yyyymmdd)) {
							drop_8ws_tmp_yyyymmdd = drop_8ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_8week);
					...
					case "9":
						String retains_hour_9week = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_active_user_retain_ws_" + retains_hour_9week, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert9WsSelectSql)) {
							insert9WsSelectSql = insert9WsSelectSql.replace("st_active_user_retain_ws_yyyymmdd", "st_active_user_retain_ws_" + retains_hour_9week);
							insert9WsSelectSql = insert9WsSelectSql.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_9week);
						...
						if (!StringUtil.isNullOrEmpty(drop_9ws_tmp_yyyymmdd)) {
							drop_9ws_tmp_yyyymmdd = drop_9ws_tmp_yyyymmdd.replace("tmp_st_active_user_retain_ws_yyyymmdd", "tmp_st_active_user_retain_ws_" + retains_hour_9week);
