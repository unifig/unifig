 ...
 */
@Service
public class Dw_ApplicationErrorReport extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(Dw_ApplicationErrorReport.class);
	private static final String configJsonPath = "classpath*:conf/json/dw_applicationErrorReport.json";
	...
	public String getName() {
		return "dw.applicationErrorReport";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_sql)) {
				target_sql = target_sql.replace("dw_error_report_dm_yyyymm", "dw_error_report_dm_" + optime_month);
			...
			if(!StringUtil.isNullOrEmpty(delete_yes_date)){
				delete_yes_date = delete_yes_date.replace("dw_error_report_dm_yyyymm", "dw_error_report_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(inselect_ds_sql)) {
				inselect_ds_sql = inselect_ds_sql.replaceAll("dw_error_report_dm_yyyymm", "dw_error_report_dm_" + optime_month);
				inselect_ds_sql = inselect_ds_sql.replace("dwd_error_report_dm_yyyymm", "dwd_error_report_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(delete_table) && !StringUtil.isNullOrEmpty(delete_date) && saveDays.isPresent()) {
				String delSQl = super.getDelHistorySql(delete_table, delete_date, "dw_error_report_dm_", saveDays.get());
