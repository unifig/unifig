package etl.dispatch.java.spap.dwd;
...
@Service
public class Dwd_SpapLoginLog extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(Dwd_SpapLoginLog.class);
	private static final String configJsonPath = "classpath*:conf/spapjson/dwd_SpapLoginLog.json";
	private static final String select_source_storeId = "${store_id}";
	private static final String source_ods1_yyyymmdd = "ods_login_log_dm_yyyymm";
	private static final String source_ods2_yyyymmdd = "ods_user_info_dm_yyyymm";
	private static final String target_dwd_yyyymmdd = "dwd_login_log_dm_yyyymm";
	private DataSourcePool dataSourcePool;
	private Optional<Integer> saveDays = Optional.empty();

	public String getName() {
		return "Dwd_SpapLoginLog";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_sql)) {
				target_sql = target_sql.replace(target_dwd_yyyymmdd, "dwd_login_log_dm_" + optime_month);
			...
			if(!StringUtil.isNullOrEmpty(delete_yes_date)){
				delete_yes_date = delete_yes_date.replace("dwd_login_log_dm_yyyymm", "dwd_login_log_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(inselect_sql)) {
				inselect_sql = inselect_sql.replace(source_ods1_yyyymmdd, "ods_login_log_dm_" + optime_month);
				inselect_sql = inselect_sql.replace(source_ods2_yyyymmdd, "ods_user_info_dm_" + optime_month);
				inselect_sql = inselect_sql.replace(target_dwd_yyyymmdd, "dwd_login_log_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(delete_table) && !StringUtil.isNullOrEmpty(delete_date) && saveDays.isPresent()) {
				String delSql = super.getDelHistorySql(delete_table, delete_date, "dwd_login_log_dm_", saveDays.get());
