...
import etl.dispatch.java.spap.ods.dao.SpapDimIpDao;
...
@Service
public class SpapOdsDimIpServiceImpl implements SpapOdsDimIpService {
	private final static Logger logger = LoggerFactory.getLogger(SpapOdsDimIpServiceImpl.class);
	...
	private final static SpapDimIp NULL_NOT_PRESENT = new SpapDimIp(); 
	
	@Autowired
	private SpapDimDataSource dimDataSource;
	
	@Autowired
	private SpapDimIpDao dimIpDao;
	private LoadingCache<DimIpKey, SpapDimIp> dimIpKeyCache;
	private Lock lock;
	
	public SpapOdsDimIpServiceImpl() {
	...
	@Override
	public SpapDimIp getDimIpByDimIp(SpapDimIp ip, boolean createIfNotExist) {
		if (null == ip) {
			return null;
		}
	...
	public SpapDimIp findDimIpByIp(String ip, long ipNum) {
		...
		List<SpapDimIp> rs = this.dimIpDao.findDimIpByIp(dimDataSource, new NewMapUtil("ip", ip).set("ipNum", ipNum).get());
	...
	private static class DimIpKeyCacheLoader extends CacheLoader<DimIpKey, SpapDimIp> {
		private SpapOdsDimIpService odsDimIpService;

		DimIpKeyCacheLoader(SpapOdsDimIpService odsDimIpService) {
			this.odsDimIpService = odsDimIpService;
		}

		@Override
		public SpapDimIp load(DimIpKey ipKey) throws Exception {
			SpapDimIp dimIp = this.odsDimIpService.findDimIpByIp(ipKey.ip, ipKey.ipNum);
			return dimIp == null ? NULL_NOT_PRESENT : (SpapDimIp) dimIp.clone();
		}
	}
}
