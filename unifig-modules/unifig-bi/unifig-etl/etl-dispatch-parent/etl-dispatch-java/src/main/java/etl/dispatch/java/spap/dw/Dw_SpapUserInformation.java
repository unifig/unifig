...
/**
 * 注册用户数据轻度汇总（数据特殊，存储全量的数据）
 *
 */
@Service
public class Dw_SpapUserInformation extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(Dw_SpapUserInformation.class);
	private static final String configJsonPath = "classpath*:conf/spapjson/dw_SpapUserInformation.json";
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(inselect_ds_sql)) {
				inselect_ds_sql = inselect_ds_sql.replace("dwd_user_info_dm_yyyymm", "dwd_user_info_dm_" + optime_month);
				...
				inselect_ds_sql = inselect_ds_sql.replace("${is_new}", String.valueOf("0"));//0-新用户 1-老用户
