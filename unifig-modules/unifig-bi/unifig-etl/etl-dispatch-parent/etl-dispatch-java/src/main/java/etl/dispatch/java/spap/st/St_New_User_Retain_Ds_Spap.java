 ...
 */
@Service
public class St_New_User_Retain_Ds_Spap extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(St_New_User_Retain_Ds_Spap.class);
	private static final String configJsonPath = "classpath*:conf/spapjson/st_new_user_retain_ds.json";
	private static final String select_source_storeId = "${store_id}";

	private DataSourcePool dataSourcePool;

	public String getName() {
		return "st.new_user_retain_ds_spap";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if(!StringUtil.isNullOrEmpty(target_dm_sql)){
				target_dm_sql = target_dm_sql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + optime_month);
			...
			if(!StringUtil.isNullOrEmpty(delete_yes_date)){
				delete_yes_date = delete_yes_date.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + optime_month);
			...
			/**
			 * 日留存统计>>>>>> st_new_user_retain_ds_yyyymmdd
			 */
			// 插入单日新用户数据
			...
			if (!StringUtil.isNullOrEmpty(insert_ds_tmpuser_yyyymmdd)) {
				...
				insert_ds_tmpuser_yyyymmdd = insert_ds_tmpuser_yyyymmdd.replace("${is_new}", String.valueOf("0"));//0-新用户 1-老用户
			...
			if (!StringUtil.isNullOrEmpty(insertDsSql)) {
				insertDsSql = insertDsSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + optime_month);
			}
			// 获取日留存查询SQL
			...
			if (!StringUtil.isNullOrEmpty(selectDsSql)) {
				...
				selectDsSql = selectDsSql.replace("${is_new}", String.valueOf("0"));//0-新用户 1-老用户
			}
			// 获取日留存查询聚合SQL
			...
			// 历史新用户留存统计数据，2、3、4、5、6、7、15、30
			Integer[] retainTimes = new Integer[] { 2, 3, 4, 5, 6, 7, 15, 30 };

			// 查询新用户时间
			String statisDateSql = "select statis_date from bi_tmp_spap.tmp_st_new_user_retain_ds_all_store group by statis_date";
			...
			for (int retainTime : retainTimes) {
				...
				String retains_hour_month = retains_hour_day.substring(0,retains_hour_day.length() - 2);
				...
				if (!StringUtil.isNullOrEmpty(drop_ds_tmp_yyyymmdd)) {
					drop_ds_tmp_yyyymmdd = drop_ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_day);
				...
				if (!StringUtil.isNullOrEmpty(create_ds_tmp_yyyymmdd)) {
					create_ds_tmp_yyyymmdd = create_ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_day);
				...
				if (!StringUtil.isNullOrEmpty(insert_ds_old_yyyymmdd)) {
					insert_ds_old_yyyymmdd = insert_ds_old_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_day);
					insert_ds_old_yyyymmdd = insert_ds_old_yyyymmdd.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_month);
				...
				for (String retainDate : retainUserDate) {
					...
					case "2":
						...
						if (!StringUtil.isNullOrEmpty(insert_2ds_new_yyyymmdd)) {
							insert_2ds_new_yyyymmdd = insert_2ds_new_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_2day);
						}
						// 获取日留存查询SQL
					...
					case "3":
						...
						if (!StringUtil.isNullOrEmpty(insert_3ds_new_yyyymmdd)) {
							insert_3ds_new_yyyymmdd = insert_3ds_new_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_3day);
						}
						// 获取日留存查询SQL
					...
					case "4":
						...
						if (!StringUtil.isNullOrEmpty(insert_4ds_new_yyyymmdd)) {
							insert_4ds_new_yyyymmdd = insert_4ds_new_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_4day);
						}
						// 获取日留存查询SQL
					...
					case "5":
						...
						if (!StringUtil.isNullOrEmpty(insert_5ds_new_yyyymmdd)) {
							insert_5ds_new_yyyymmdd = insert_5ds_new_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_5day);
						}
						// 获取日留存查询SQL
					...
					case "6":
						...
						if (!StringUtil.isNullOrEmpty(insert_6ds_new_yyyymmdd)) {
							insert_6ds_new_yyyymmdd = insert_6ds_new_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_6day);
						}
						// 获取日留存查询SQL
					...
					case "7":
						...
						if (!StringUtil.isNullOrEmpty(insert_7ds_new_yyyymmdd)) {
							insert_7ds_new_yyyymmdd = insert_7ds_new_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_7day);
						}
						// 获取日留存查询SQL
					...
					case "15":
						...
						if (!StringUtil.isNullOrEmpty(insert_15ds_new_yyyymmdd)) {
							insert_15ds_new_yyyymmdd = insert_15ds_new_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_15day);
						}
						// 获取日留存查询SQL
					...
					case "30":
						...
						if (!StringUtil.isNullOrEmpty(insert_30ds_new_yyyymmdd)) {
							insert_30ds_new_yyyymmdd = insert_30ds_new_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_30day);
						}
						// 获取日留存查询SQL
			...
			if (!retainUserDate.isEmpty()) {
				for (String retainDate : retainUserDate) {
					...
					case "2":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_new_user_retain_dm_" + retains_hour_2month + " WHERE statis_date = " + retains_hour_2day, this.getName());
						// 插入新聚合数据
						String insert2DsSelectSql = super.getJsonConfigValue(configJsonPath, "insertDsSelectSql");
						if (!StringUtil.isNullOrEmpty(insert2DsSelectSql)) {
							insert2DsSelectSql = insert2DsSelectSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_2month);
							insert2DsSelectSql = insert2DsSelectSql.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_2day);
						...
						if (!StringUtil.isNullOrEmpty(drop_2ds_tmp_yyyymmdd)) {
							drop_2ds_tmp_yyyymmdd = drop_2ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_2day);
					...
					case "3":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_new_user_retain_dm_" + retains_hour_3month + " WHERE statis_date = " + retains_hour_3day, this.getName());
						// 插入新聚合数据
						String insert3DsSelectSql = super.getJsonConfigValue(configJsonPath, "insertDsSelectSql");
						if (!StringUtil.isNullOrEmpty(insert3DsSelectSql)) {
							insert3DsSelectSql = insert3DsSelectSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_3month);
							insert3DsSelectSql = insert3DsSelectSql.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_3day);
						...
						if (!StringUtil.isNullOrEmpty(drop_3ds_tmp_yyyymmdd)) {
							drop_3ds_tmp_yyyymmdd = drop_3ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_3day);
					...
					case "4":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_new_user_retain_dm_" + retains_hour_4month + " WHERE statis_date = " + retains_hour_4day, this.getName());
						// 插入新聚合数据
						String insert4DsSelectSql = super.getJsonConfigValue(configJsonPath, "insertDsSelectSql");
						if (!StringUtil.isNullOrEmpty(insert4DsSelectSql)) {
							insert4DsSelectSql = insert4DsSelectSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_4month);
							insert4DsSelectSql = insert4DsSelectSql.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_4day);
						...
						if (!StringUtil.isNullOrEmpty(drop_4ds_tmp_yyyymmdd)) {
							drop_4ds_tmp_yyyymmdd = drop_4ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_4day);
					...
					case "5":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_new_user_retain_dm_" + retains_hour_5month + " WHERE statis_date = " + retains_hour_5day, this.getName());
						// 插入新聚合数据
						String insert5DsSelectSql = super.getJsonConfigValue(configJsonPath, "insertDsSelectSql");
						if (!StringUtil.isNullOrEmpty(insert5DsSelectSql)) {
							insert5DsSelectSql = insert5DsSelectSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_5month);
							insert5DsSelectSql = insert5DsSelectSql.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_5day);
						...
						if (!StringUtil.isNullOrEmpty(drop_5ds_tmp_yyyymmdd)) {
							drop_5ds_tmp_yyyymmdd = drop_5ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_5day);
					...
					case "6":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_new_user_retain_dm_" + retains_hour_6month + " WHERE statis_date = " + retains_hour_6day, this.getName());
						// 插入新聚合数据
						String insert6DsSelectSql = super.getJsonConfigValue(configJsonPath, "insertDsSelectSql");
						if (!StringUtil.isNullOrEmpty(insert6DsSelectSql)) {
							insert6DsSelectSql = insert6DsSelectSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_6month);
							insert6DsSelectSql = insert6DsSelectSql.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_6day);
						...
						if (!StringUtil.isNullOrEmpty(drop_6ds_tmp_yyyymmdd)) {
							drop_6ds_tmp_yyyymmdd = drop_6ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_6day);
					...
					case "7":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_new_user_retain_dm_" + retains_hour_7month + " WHERE statis_date = " + retains_hour_7day, this.getName());
						// 插入新聚合数据
						String insert7DsSelectSql = super.getJsonConfigValue(configJsonPath, "insertDsSelectSql");
						if (!StringUtil.isNullOrEmpty(insert7DsSelectSql)) {
							insert7DsSelectSql = insert7DsSelectSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_7month);
							insert7DsSelectSql = insert7DsSelectSql.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_7day);
						...
						if (!StringUtil.isNullOrEmpty(drop_7ds_tmp_yyyymmdd)) {
							drop_7ds_tmp_yyyymmdd = drop_7ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_7day);
					...
					case "15":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_new_user_retain_dm_" + retains_hour_15month + " WHERE statis_date = " + retains_hour_15day, this.getName());
						// 插入新聚合数据
						String insert15DsSelectSql = super.getJsonConfigValue(configJsonPath, "insertDsSelectSql");
						if (!StringUtil.isNullOrEmpty(insert15DsSelectSql)) {
							insert15DsSelectSql = insert15DsSelectSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_15month);
							insert15DsSelectSql = insert15DsSelectSql.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_15day);
						...
						if (!StringUtil.isNullOrEmpty(drop_15ds_tmp_yyyymmdd)) {
							drop_15ds_tmp_yyyymmdd = drop_15ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_15day);
					...
					case "30":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_new_user_retain_dm_" + retains_hour_30month + " WHERE statis_date = " + retains_hour_30day, this.getName());
						// 插入新聚合数据
						String insert30DsSelectSql = super.getJsonConfigValue(configJsonPath, "insertDsSelectSql");
						if (!StringUtil.isNullOrEmpty(insert30DsSelectSql)) {
							insert30DsSelectSql = insert30DsSelectSql.replace("st_new_user_retain_dm_yyyymm", "st_new_user_retain_dm_" + retains_hour_30month);
							insert30DsSelectSql = insert30DsSelectSql.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_30day);
						...
						if (!StringUtil.isNullOrEmpty(drop_30ds_tmp_yyyymmdd)) {
							drop_30ds_tmp_yyyymmdd = drop_30ds_tmp_yyyymmdd.replace("tmp_st_new_user_retain_ds_yyyymmdd", "tmp_st_new_user_retain_ds_" + retains_hour_30day);
