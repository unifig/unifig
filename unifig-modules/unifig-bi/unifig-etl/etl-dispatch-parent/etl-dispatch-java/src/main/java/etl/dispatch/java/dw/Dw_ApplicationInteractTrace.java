 ...
 */
@Service
public class Dw_ApplicationInteractTrace extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(Dw_ApplicationInteractTrace.class);
	private static final String configJsonPath = "classpath*:conf/json/dw_applicationInteractTrace.json";
	...
	public String getName() {
		return "dw.applicationInteractTrace";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_sql)) {
				target_sql = target_sql.replace("dw_interact_trace_dm_yyyymm", "dw_interact_trace_dm_" + optime_month);
			...
			if(!StringUtil.isNullOrEmpty(delete_yes_date)){
				delete_yes_date = delete_yes_date.replace("dw_interact_trace_dm_yyyymm", "dw_interact_trace_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(inselect_ds_sql)) {
				inselect_ds_sql = inselect_ds_sql.replace("dw_interact_trace_dm_yyyymm", "dw_interact_trace_dm_" + optime_month);
				inselect_ds_sql = inselect_ds_sql.replace("dwd_interact_trace_dm_yyyymm", "dwd_interact_trace_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(delete_table) && !StringUtil.isNullOrEmpty(delete_date) && saveDays.isPresent()) {
				String delSQl = super.getDelHistorySql(delete_table, delete_date, "dwd_interact_trace_dm_", saveDays.get());
