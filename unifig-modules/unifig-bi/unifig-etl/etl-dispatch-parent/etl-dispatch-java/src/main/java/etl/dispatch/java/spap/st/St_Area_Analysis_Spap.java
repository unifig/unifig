 ...
 */
@Service
public class St_Area_Analysis_Spap extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(St_Area_Analysis_Spap.class);
	private static final String configJsonPath = "classpath*:conf/spapjson/st_area_analysis_spap.json";
	private static final String select_source_storeId = "${store_id}";

	private DataSourcePool dataSourcePool;

	public String getName() {
		return "st_Area_Analysis_Spap";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			for (List<String> listRollup : splitDsNewUserList) {
				...
				if (null != listRollup && !listRollup.isEmpty()) {
					...
					for (String withRollup : listRollup) {
						...
						strDsBuffer.append(" group by `statis_date`,  channel," + withRollup.substring(1) + "  with rollup ");
						i++;
					}
				}
				// 插入(DS) Cube聚合数据
			...
			// 清空临时表
			SqlUtils.sqlExecute(dataSource, "truncate table bi_tmp_spap.tmp_st_area_analysis_ds_" + ScriptTimeUtil.optime_yesday(), this.getName());
			// 数据迁移到临时表
			...
			// 清空DS表
			SqlUtils.sqlExecute(dataSource, "delete from bi_st_spap.st_area_analysis_dm_" + optime_month + " where statis_date = " + ScriptTimeUtil.optime_yesday(), this.getName());
			// 聚合数据
			...
			// 清空临时表
			SqlUtils.sqlExecute(dataSource, "truncate table bi_tmp_spap.tmp_st_area_analysis_dt_" + ScriptTimeUtil.optime_yesday(), this.getName());
			// 数据迁移到临时表
			...
			// 清空DT表
			SqlUtils.sqlExecute(dataSource, "delete from bi_st_spap.st_area_analysis_dm_" + optime_month + " where time_slice != 1 and statis_date = " + ScriptTimeUtil.optime_yesday(), this.getName());
			// 聚合数据
