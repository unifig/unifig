 ...
 */
@Service
public class Dwd_ApplicationEventReport extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(Dwd_ApplicationEventReport.class);
	private static final String configJsonPath = "classpath*:conf/json/dwd_applicationEventReport.json";
	private static final String select_source_storeId = "${store_id}";
	private static final String target_dwd_yyyymmdd = "dwd_event_report_dm_yyyymm";
	private DataSourcePool dataSourcePool;
	private Optional<Integer> saveDays = Optional.empty();

	public String getName() {
		return "dwd.applicationEventReport";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_sql)) {
				target_sql = target_sql.replace(target_dwd_yyyymmdd, "dwd_event_report_dm_" + optime_month);
			...
			if(!StringUtil.isNullOrEmpty(delete_yes_date)){
				delete_yes_date = delete_yes_date.replace("dwd_event_report_dm_yyyymm", "dwd_event_report_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(inselect_sql)) {
				inselect_sql = inselect_sql.replace(target_dwd_yyyymmdd, "dwd_event_report_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(delete_table) && !StringUtil.isNullOrEmpty(delete_date) && saveDays.isPresent()) {
				String delSQls = super.getDelHistorySql(delete_table, delete_date, "dwd_event_report_dm_", saveDays.get());
