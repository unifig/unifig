 ...
 */
@Service
public class St_Overview_Analysis_spap extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(St_Overview_Analysis_spap.class);
	private static final String configJsonPath = "classpath*:conf/spapjson/st_overview_analysis_spap.json";
	private static final String select_source_storeId = "${store_id}";

	private DataSourcePool dataSourcePool;

	public String getName() {
		return "st.st_Overview_Analysis_spap";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			// 查询一天用户数据
			List<Map> dayUserCountMap = null;
			String select_dt_Dusercount = super.getJsonConfigValue(configJsonPath, "select_dt_WMusercount");
			if (!StringUtil.isNullOrEmpty(select_dt_Dusercount)) {
				select_dt_Dusercount = select_dt_Dusercount.replace("${min_id}", DateUtil.getSysStrCurrentDate("yyyyMMdd", -1));
				select_dt_Dusercount = select_dt_Dusercount.replace("${max_id}", DateUtil.getSysStrCurrentDate("yyyyMMdd", -1));
				dayUserCountMap = SqlUtils.querySqlList(dataSource, select_dt_Dusercount, this.getName());
			}
			...
			if (!StringUtil.isNullOrEmpty(insertDtValuesSql)) {
				...
				insertDtValuesSql = insertDtValuesSql.replace("${day_user_count}", null == dayUserCountMap ? "0" : String.valueOf(dayUserCountMap.get(0).get("user_count")));
