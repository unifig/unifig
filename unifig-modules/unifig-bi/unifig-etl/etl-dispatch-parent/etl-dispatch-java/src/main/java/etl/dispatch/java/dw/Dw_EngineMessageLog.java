...
@Service
public class Dw_EngineMessageLog extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(Dw_EngineMessageLog.class);
	private static final String configJsonPath = "classpath*:conf/json/dw_engineMessage.json";
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_ds_sql)) {
				target_ds_sql = target_ds_sql.replace("dw_engine_message_log_dm_yyyymm", "dw_engine_message_log_dm_" + optime_month);
			...
			if(!StringUtil.isNullOrEmpty(delete_yes_date)){
				delete_yes_date = delete_yes_date.replace("dw_engine_message_log_dm_yyyymm", "dw_engine_message_log_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(inselect_ds_sql)) {
				inselect_ds_sql = inselect_ds_sql.replace("dw_engine_message_log_dm_yyyymm", "dw_engine_message_log_dm_" + optime_month);
				inselect_ds_sql = inselect_ds_sql.replace("dwd_engine_message_log_dm_yyyymm", "dwd_engine_message_log_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(delete_table) && !StringUtil.isNullOrEmpty(delete_date) && saveDays.isPresent()) {
				String delSQl = super.getDelHistorySql(delete_table, delete_date, "dw_engine_message_log_dm_", saveDays.get());
