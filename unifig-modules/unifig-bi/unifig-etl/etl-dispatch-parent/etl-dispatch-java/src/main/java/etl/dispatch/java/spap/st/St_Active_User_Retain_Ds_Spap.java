 ...
 */
@Service
public class St_Active_User_Retain_Ds_Spap extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(St_Active_User_Retain_Ds_Spap.class);
	private static final String configJsonPath = "classpath*:conf/spapjson/st_active_user_retain_ds_spap.json";
	private static final String select_source_storeId = "${store_id}";

	private DataSourcePool dataSourcePool;

	public String getName() {
		return "st.st_active_user_retain_ds_spap";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try { 
			...
			// 查询活跃用户时间
			String statisDateSql = "select statis_date from bi_tmp_spap.tmp_st_active_user_retain_ds_all_store group by statis_date";
			...
			if (!retainUserDate.isEmpty()) {
				for (String retainDate : retainUserDate) {
					...
					case "2":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_active_user_retain_dm_" + retains_hour_2month + " where statis_date = " + retains_hour_2day, this.getName());
						// 插入新聚合数据
					...
					case "3":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_active_user_retain_dm_" + retains_hour_3month + " where statis_date = " + retains_hour_3day, this.getName());
						// 插入新聚合数据
					...
					case "4":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_active_user_retain_dm_" + retains_hour_4month + " where statis_date = " + retains_hour_4day, this.getName());
						// 插入新聚合数据
					...
					case "5":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_active_user_retain_dm_" + retains_hour_5month + " where statis_date = " + retains_hour_5day, this.getName());
						// 插入新聚合数据
					...
					case "6":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_active_user_retain_dm_" + retains_hour_6month + " where statis_date = " + retains_hour_6day, this.getName());
						// 插入新聚合数据
					...
					case "7":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_active_user_retain_dm_" + retains_hour_7month + " where statis_date = " + retains_hour_7day, this.getName());
						// 插入新聚合数据
					...
					case "15":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_active_user_retain_dm_" + retains_hour_15month + " where statis_date = " + retains_hour_15day, this.getName());
						// 插入新聚合数据
					...
					case "30":
						...
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "DELETE FROM bi_st_spap.st_active_user_retain_dm_" + retains_hour_30month + " where statis_date = " + retains_hour_30day, this.getName());
						// 插入新聚合数据
