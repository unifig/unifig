...
import etl.dispatch.java.ods.Ods_ApplicationLoginLog;
 ...
 */
@Service
public class Dwd_ApplicationErrorReport extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(Dwd_ApplicationErrorReport.class);
	private static final String configJsonPath = "classpath*:conf/json/dwd_applicationErrorReport.json";
	private static final String select_source_storeId = "${store_id}";
	private static final String target_dwd_yyyymmdd = "dwd_error_report_dm_yyyymm";
	private DataSourcePool dataSourcePool;
	private Optional<Integer> saveDays = Optional.empty();

	public String getName() {
		return "dwd.applicationErrorReport";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_sql)) {
				target_sql = target_sql.replace(target_dwd_yyyymmdd, "dwd_error_report_dm_" + optime_month);
			...
			if(!StringUtil.isNullOrEmpty(delete_yes_date)){
				delete_yes_date = delete_yes_date.replace("dwd_error_report_dm_yyyymm", "dwd_error_report_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(inselect_sql)) {
				inselect_sql = inselect_sql.replace(target_dwd_yyyymmdd, "dwd_error_report_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(delete_table) && !StringUtil.isNullOrEmpty(delete_date) && saveDays.isPresent()) {
				String delSQl = super.getDelHistorySql(delete_table, delete_date, "dwd_error_report_dm_", saveDays.get());
