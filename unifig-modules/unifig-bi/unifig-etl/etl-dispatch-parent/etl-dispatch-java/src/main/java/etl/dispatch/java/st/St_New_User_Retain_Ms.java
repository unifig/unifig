...
/**
 * 新增用户首次使用月留存 统计;月留存统计>>>>>> st_new_user_retain_ms_yyyymm
 *
 */
@Service
public class St_New_User_Retain_Ms extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(St_New_User_Retain_Ms.class);
	private static final String configJsonPath = "classpath*:conf/json/st_new_user_retain_ms.json";
	private static final String select_source_storeId = "${store_id}";

	private DataSourcePool dataSourcePool;

	public String getName() {
		return "st.new_user_retain_ms";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_ms_sql)) {
				target_ms_sql = target_ms_sql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + ScriptTimeUtil.optime_lastmonth());
			...
			if (!StringUtil.isNullOrEmpty(insert_ms_tmpuser_yyyymm)) {
				...
				insert_ms_tmpuser_yyyymm = insert_ms_tmpuser_yyyymm.replace("${is_new}", String.valueOf("0"));//0-新用户 1-老用户
			...
			if (!StringUtil.isNullOrEmpty(insertMsSql)) {
				insertMsSql = insertMsSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + ScriptTimeUtil.optime_lastmonth());
			}
			// 获取月留存查询SQL
			...
			if (!StringUtil.isNullOrEmpty(selectMsSql)) {
				...
				selectMsSql = selectMsSql.replace("${is_new}", String.valueOf("0"));//0-新用户 1-老用户
			}
			// 获取月留存查询聚合SQL
			...
			// 查询新用户时间
			String statisDateSql = "select statis_date from bi_tmp.tmp_st_new_user_retain_ms_all_store group by statis_date";
			...
			for (int retainTime : retainTimes) {
				...
				if (!StringUtil.isNullOrEmpty(drop_ms_tmp_yyyymm)) {
					drop_ms_tmp_yyyymm = drop_ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_month);
				...
				if (!StringUtil.isNullOrEmpty(create_ms_tmp_yyyymm)) {
					create_ms_tmp_yyyymm = create_ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_month);
				...
				if (!StringUtil.isNullOrEmpty(insert_ms_old_yyyymm)) {
					insert_ms_old_yyyymm = insert_ms_old_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_month);
					insert_ms_old_yyyymm = insert_ms_old_yyyymm.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_month);
				...
				for (String retainDate : retainUserDate) {
					...
					case "2":
						...
						if (!StringUtil.isNullOrEmpty(insert_2ms_new_yyyymm)) {
							insert_2ms_new_yyyymm = insert_2ms_new_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_2month);
						}
						// 获取月留存查询SQL
					...
					case "3":
						...
						if (!StringUtil.isNullOrEmpty(insert_3ms_new_yyyymm)) {
							insert_3ms_new_yyyymm = insert_3ms_new_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_3month);
						}
						// 获取月留存查询SQL
					...
					case "4":
						...
						if (!StringUtil.isNullOrEmpty(insert_4ms_new_yyyymm)) {
							insert_4ms_new_yyyymm = insert_4ms_new_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_4month);
						}
						// 获取月留存查询SQL
					...
					case "5":
						...
						if (!StringUtil.isNullOrEmpty(insert_5ms_new_yyyymm)) {
							insert_5ms_new_yyyymm = insert_5ms_new_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_5month);
						}
						// 获取月留存查询SQL
					...
					case "6":
						...
						if (!StringUtil.isNullOrEmpty(insert_6ms_new_yyyymm)) {
							insert_6ms_new_yyyymm = insert_6ms_new_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_6month);
						}
						// 获取月留存查询SQL
					...
					case "7":
						...
						if (!StringUtil.isNullOrEmpty(insert_7ms_new_yyyymm)) {
							insert_7ms_new_yyyymm = insert_7ms_new_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_7month);
						}
						// 获取月留存查询SQL
					...
					case "8":
						...
						if (!StringUtil.isNullOrEmpty(insert_8ms_new_yyyymm)) {
							insert_8ms_new_yyyymm = insert_8ms_new_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_8month);
						}
						// 获取月留存查询SQL
					...
					case "9":
						...
						if (!StringUtil.isNullOrEmpty(insert_9ms_new_yyyymm)) {
							insert_9ms_new_yyyymm = insert_9ms_new_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_9month);
						}
						// 获取月留存查询SQL
			...
			if (!retainUserDate.isEmpty()) {
				for (String retainDate : retainUserDate) {
					...
					case "2":
						String retains_hour_2month = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_new_user_retain_ms_" + retains_hour_2month, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert2MsSelectSql)) {
							insert2MsSelectSql = insert2MsSelectSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_2month);
							insert2MsSelectSql = insert2MsSelectSql.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_2month);
						...
						if (!StringUtil.isNullOrEmpty(drop_2ms_tmp_yyyymm)) {
							drop_2ms_tmp_yyyymm = drop_2ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_2month);
					...
					case "3":
						String retains_hour_3month = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_new_user_retain_ms_" + retains_hour_3month, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert3MsSelectSql)) {
							insert3MsSelectSql = insert3MsSelectSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_3month);
							insert3MsSelectSql = insert3MsSelectSql.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_3month);
						...
						if (!StringUtil.isNullOrEmpty(drop_3ms_tmp_yyyymm)) {
							drop_3ms_tmp_yyyymm = drop_3ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_3month);
					...
					case "4":
						String retains_hour_4month = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_new_user_retain_ms_" + retains_hour_4month, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert4MsSelectSql)) {
							insert4MsSelectSql = insert4MsSelectSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_4month);
							insert4MsSelectSql = insert4MsSelectSql.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_4month);
						...
						if (!StringUtil.isNullOrEmpty(drop_4ms_tmp_yyyymm)) {
							drop_4ms_tmp_yyyymm = drop_4ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_4month);
					...
					case "5":
						String retains_hour_5month = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_new_user_retain_ms_" + retains_hour_5month, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert5MsSelectSql)) {
							insert5MsSelectSql = insert5MsSelectSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_5month);
							insert5MsSelectSql = insert5MsSelectSql.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_5month);
						...
						if (!StringUtil.isNullOrEmpty(drop_5ms_tmp_yyyymm)) {
							drop_5ms_tmp_yyyymm = drop_5ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_5month);
					...
					case "6":
						String retains_hour_6month = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_new_user_retain_ms_" + retains_hour_6month, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert6MsSelectSql)) {
							insert6MsSelectSql = insert6MsSelectSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_6month);
							insert6MsSelectSql = insert6MsSelectSql.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_6month);
						...
						if (!StringUtil.isNullOrEmpty(drop_6ms_tmp_yyyymm)) {
							drop_6ms_tmp_yyyymm = drop_6ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_6month);
					...
					case "7":
						String retains_hour_7month = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_new_user_retain_ms_" + retains_hour_7month, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert7MsSelectSql)) {
							insert7MsSelectSql = insert7MsSelectSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_7month);
							insert7MsSelectSql = insert7MsSelectSql.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_7month);
						...
						if (!StringUtil.isNullOrEmpty(drop_7ms_tmp_yyyymm)) {
							drop_7ms_tmp_yyyymm = drop_7ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_7month);
					...
					case "8":
						String retains_hour_8month = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_new_user_retain_ms_" + retains_hour_8month, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert8MsSelectSql)) {
							insert8MsSelectSql = insert8MsSelectSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_8month);
							insert8MsSelectSql = insert8MsSelectSql.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_8month);
						...
						if (!StringUtil.isNullOrEmpty(drop_8ms_tmp_yyyymm)) {
							drop_8ms_tmp_yyyymm = drop_8ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_8month);
					...
					case "9":
						String retains_hour_9month = retains[1];
						// truncate旧 数据
						SqlUtils.sqlExecute(dataSource, "truncate table bi_st.st_new_user_retain_ms_" + retains_hour_9month, this.getName());
						// 插入新聚合数据
						...
						if (!StringUtil.isNullOrEmpty(insert9MsSelectSql)) {
							insert9MsSelectSql = insert9MsSelectSql.replace("st_new_user_retain_ms_yyyymm", "st_new_user_retain_ms_" + retains_hour_9month);
							insert9MsSelectSql = insert9MsSelectSql.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_9month);
						...
						if (!StringUtil.isNullOrEmpty(drop_9ms_tmp_yyyymm)) {
							drop_9ms_tmp_yyyymm = drop_9ms_tmp_yyyymm.replace("tmp_st_new_user_retain_ms_yyyymm", "tmp_st_new_user_retain_ms_" + retains_hour_9month);
