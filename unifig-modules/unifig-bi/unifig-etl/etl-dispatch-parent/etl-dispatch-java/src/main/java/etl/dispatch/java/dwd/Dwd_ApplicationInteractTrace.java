 ...
 */
@Service
public class Dwd_ApplicationInteractTrace extends AbstractScript {
	private static Logger logger = LoggerFactory.getLogger(Dwd_ApplicationInteractTrace.class);
	private static final String configJsonPath = "classpath*:conf/json/dwd_applicationInteractTrace.json";
	private static final String select_source_storeId = "${store_id}";
	private static final String target_dwd_yyyymmdd = "dwd_interact_trace_dm_yyyymm";
	private DataSourcePool dataSourcePool;
	private Optional<Integer> saveDays = Optional.empty();

	public String getName() {
		return "dwd.applicationInteractTrace";
	}
	...
	protected void start(ScriptBean scriptBean, ScriptCallBack callback) {
		...
		try {
			...
			if (!StringUtil.isNullOrEmpty(target_sql)) {
				target_sql = target_sql.replace(target_dwd_yyyymmdd, "dwd_interact_trace_dm_" + optime_month);
			...
			if(!StringUtil.isNullOrEmpty(delete_yes_date)){
				delete_yes_date = delete_yes_date.replace("dwd_interact_trace_dm_yyyymm", "dwd_interact_trace_dm_" + optime_month);
			...
			if (!StringUtil.isNullOrEmpty(inselect_sql)) {
				inselect_sql = inselect_sql.replace(target_dwd_yyyymmdd, "dwd_interact_trace_dm_" + optime_month);
