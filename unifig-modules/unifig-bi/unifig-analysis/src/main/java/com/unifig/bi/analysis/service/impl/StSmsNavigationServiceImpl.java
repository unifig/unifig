...
import com.unifig.bi.analysis.mapper.StSmsNavigationMapper;
 ...
 */
@Service
public class StSmsNavigationServiceImpl extends ServiceImpl<StSmsNavigationMapper, StSmsNavigation> implements StSmsNavigationService {
    @Autowired
    private StSmsNavigationMapper stCmsBannerMapper;

    @Override
    public Map<String, Object> line(Date startDate, Date stopDate, String navigationId) {
        ...
        if (day == 0) {
            ...
            paremMap.put("tableName", "st_sms_navigation");
            if (StringUtils.isNotEmpty(navigationId)) {
                paremMap.put("navigationId", navigationId);
            ...
            result = AnanlysisUtils.getAllHour(result, new NewMapUtil().set("statisDate", date).set("joinNavigationCount", 0)
                    .get(), null);
        ...
        result = AnanlysisUtils.getAllDate(result, day, date, new NewMapUtil().set("statisDate", date).set("joinNavigationCount", 0)
                .get(), "statisDate");
    ...
    private Map<String, Object> ResultActiveUser(List<Map<String, Object>> result, boolean isDay) {
        ...
        result.forEach(re -> {
            ...
            readArticleCount.add(Integer.valueOf(re.get("joinNavigationCount").toString()));
        });
        ...
        // 阅读量
        map.put("joinNavigationCount", signinUser);
        return map;
    }
}
