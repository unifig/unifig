...
public class PmsProductServiceImpl implements PmsProductService {
    ...
    @Autowired
    private UmsProductCollectionFeign umsProductCollectionFeign;
    ...
    public List<PmsProduct> list(PmsProductQueryParam productQueryParam, Integer pageSize, Integer pageNum) {
        ...
        if (productQueryParam.getShopId() != null) {
            criteria.andShopIdEqualTo(productQueryParam.getShopId());
        }
    ...
    public ResultData selectById(Long id) {
        PmsProductVo pmsProductVo = productMapper.selectById(id);
        boolean b = umsProductCollectionFeign.selectByProductId(id);
        pmsProductVo.setCollect(b);
        if(pmsProductVo.getGroup().toString().equals("1")){
            ...
            wp.eq("product_id",pmsProductVo.getId());
            wp.eq("status",1);
            List<PmsGroupBuying> pmsGroupBuyings = pmsGroupBuyingService.selectList(wp);
            if(pmsGroupBuyings.size()>0){
                PmsGroupBuying pmsGroupBuying = pmsGroupBuyings.get(0);
                if(pmsGroupBuying != null)
                    pmsProductVo.setRegulation(pmsGroupBuying.getRegulation());
            }
            //设置团购价格
            pmsProductVo.setGroupPurchasePrice(groupPurchasePrice);
        }
        return ResultData.result(true).setData(pmsProductVo);
    }
    @Override
    public ResultData orcodeSelectById(Long id) {
        PmsProductVo pmsProductVo = productMapper.selectById(id);
        if(pmsProductVo.getGroup().toString().equals("1")){
            PmsSkuStockExample pmsSkuStockExample = new PmsSkuStockExample();
            PmsSkuStockExample.Criteria criteria = pmsSkuStockExample.createCriteria();
