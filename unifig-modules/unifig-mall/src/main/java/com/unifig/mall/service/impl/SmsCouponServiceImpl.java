...
import com.unifig.mall.dao.SmsCouponDao;
import com.unifig.mall.dao.SmsCouponProductCategoryRelationDao;
import com.unifig.mall.dao.SmsCouponProductRelationDao;
...
public class SmsCouponServiceImpl implements SmsCouponService {
    ...
    @Autowired
    private SmsCouponShopRelationMapper smsCouponShopRelationMapper;
    ...
    public int create(SmsCouponParam couponParam) {
        ...
        //插入优惠券和店铺关系表
        if(couponParam.getUseType().equals(3)){
            for (SmsCouponShopsRelation smsCouponShopsRelation : couponParam.getShopsRelationList()) {
                smsCouponShopsRelation.setCouponId(couponParam.getId());
                smsCouponShopRelationMapper.insert(smsCouponShopsRelation);
            }
        }
        return count;
    }
    ...
    @Override
    public void staleDated() {
        List<Long> longs = couponMapper.selectStaleDated();
        if(longs.size()<1){
            log.info("当前无过期优惠券");
            return;
        }
        for (Long couponId : longs){
            log.info("关闭过期优惠券,id{}",couponId);
            smsCouponHistoryMapper.staleDated(couponId);
        }
        log.info("关闭完成");
    }
}
