package com.unifig.organ.cache;
...
public class RegionCacheUtil implements InitializingBean {
    ...
    public static void init() {
        UmsSysRegionDao regionDao = SpringContextUtils.getBean(UmsSysRegionDao.class);
        if (null != regionDao) {
            try {
                EntityWrapper<SysRegionEntity> sysRegionEntityEntityWrapper = new EntityWrapper<SysRegionEntity>();
                sysRegionEntityList = regionDao.selectList(sysRegionEntityEntityWrapper);
            } catch (Exception e) {
                e.printStackTrace();
            }
